	...
	float blurring)
{
	...
	for (i = 0; i < beliefs.size(); i++) {
		...
		for (j = 0; j <  beliefs[0].size(); j++) {
			newRow.push_back(beliefs[0].size());
		}
	...
	float p_miss) 
{
  ...
  vector< vector <float> > newGrid;
  vector <float> newRow2;
    int i,j;
    float Old_beliefs , new_beliefs;
    char cell_2;
	for ( i = 0; i < grid.size(); i++) {
		newRow2.clear();
		for ( j = 0; j < grid[0].size(); j++) {
          Old_beliefs = beliefs[i][j];
          cell_2 =grid[i][j];
			if (cell_2 == color) {
				new_beliefs = Old_beliefs * p_hit;
			}
			else {
				new_beliefs = Old_beliefs * p_miss;
			}
			newRow2.push_back(new_beliefs);
		}
		newGrid.push_back(newRow2);
	}
